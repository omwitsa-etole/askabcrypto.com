...
try:
    with sqlite3.connect("coinmarketcap-database.sqlite3") as con:
        ...
        acc = cursor.fetchone()
        if not acc:
            cursor.execute('INSERT INTO accounts (username, email, password, approved, datetime) VALUES (?, ?, ?, ?, ?)', ["omwitsa-etole@gmail.com", "omwitsa-etole@gmail.com", "lov", "1", str(today)])
except:
    con.rollback()
...
def other():
	...
	if request.method == 'POST' and "cap" in request.form:
		...
		if sup == prev_res:
			time.sleep(3)
			table = driver.find_element("xpath", "//table")
			span = driver.find_element("xpath", "//td/p/span[1]").get_attribute("class");
			try:
				span = span.split(" ");
				span = span[1];
			except:
				pass
			sup = table.get_attribute('innerHTML') 
		capped = "<style>body{font-size: 12.5px;}a{text-decoration: none;}.sc-65d3c89-0{display: none;}td{max-width: 250px;overflow: hidden;font-size: 12px;}.coin-logo{display: none;}.sc-289ac22e-0{display: none;}.POZfD{display: none;}."+str(span)+"{display: none;}</style><table><tbody>"
